...
<head>
    ...
    <title><?php echo $settings['display_name'] ?? 'Midtrans'; ?> - <?php echo $setting['response'][0]['site_name']?></title>
    ...
    <!-- Midtrans Snap JS -->
    <script src="<?php echo (isset($settings['environment']) && $settings['environment'] === 'production') ? 'https://app.midtrans.com/snap/snap.js' : 'https://app.sandbox.midtrans.com/snap/snap.js'; ?>" data-client-key="<?php echo htmlspecialchars($settings['client_key'] ?? ''); ?>"></script>
    <style>
        :root {
            --secondary: #0066cc;
            --success: #00b894;
...
<body>
    <div class="payment-container">
        <div class="payment-header">
            <div class="payment-logo">
                ...
                <span><?php echo $settings['display_name'] ?? 'Midtrans'; ?></span>
            </div>
            <div style="font-size: 0.9rem; color: var(--gray);">Secure Payment</div>
        </div>

        <div class="payment-body">
            <div class="info-box">
                <i class="fas fa-info-circle"></i> You will be redirected to Midtrans to complete the payment securely.
            </div>
            ...
            <button class="btn-pay" id="payButton" onclick="processPayment()">
                <i class="fas fa-credit-card me-2"></i>Pay Now with Midtrans
            </button>
    ...
    <script>
        ...
        function processPayment() {
            ...
            snap.pay('<?php 
                // Generate a token (in production, this should be generated by server-side)
                // This is a placeholder - actual implementation should generate valid Snap token
                echo 'YOUR_SNAP_TOKEN_HERE'; 
            ?>', {
        ...
        function handlePaymentSuccess(result) {
            ...
            window.location.href = 'payment-status?transaction_id=<?php echo htmlspecialchars($transaction_details['response'][0]['transaction_id']); ?>&status=success';
        }
